...
const diagModuleMap={
  ...
  'Gestão de equipe':['Funcionários','Departamentos','Ponto Eletrônico','Checklists','Projetos'],
  ...
  'CRM/Vendas':['CRM','Clientes','Vendas','PDV','Fornecedores'],
  'Operações':['Gestão de Serviços','Checklists','Contratos','Projetos','Estoque'],
  'Contabilidade/Fiscal':['Contabilidade','Fiscal','Relatórios','BI com IA'],
  'TI/Integrações':['Integrações','Workflows','BI com IA','Backup']
};
...
function initDiag(){
  ...
  function showResult(){
    ...
    .catch(()=>{
      ...
      const winsMap={'Financeiro/Cobrança':['Conciliação bancária automatizada (97% match)','Cobrança recorrente via Pix + WhatsApp','Dashboard financeiro com projeção de caixa por IA'],'Comunicação interna':['Chat integrado com menções e canais por equipe','Tickets internos com SLA e rastreamento','Workflows automáticos para aprovações'],'Relatórios e dados':['BI preditivo com 4 provedores de IA','DRE e balanço gerados automaticamente','Relatórios customizados por centro de custo'],'Gestão de equipe':['Gestão de funcionários com férias e documentos','Checklists operacionais com prazos e responsáveis','Projetos com Kanban e controle de horas'],'Múltiplos sistemas':['Integração nativa com 8+ plataformas','Workflows automatizados entre módulos','API unificada substituindo múltiplas ferramentas']};
...
{t:"Firebase Security Rules na prática: como o Ruphus isola dados entre empresas",c:"Segurança",body:"<p>Cada tenant vê apenas seus dados — garantido no nível do banco de dados, não da aplicação. Firestore Security Rules + Custom Claims do Firebase Auth verificam membership em todas as operações de leitura e escrita.</p><h2>200+ permissões granulares</h2><p>O sistema de permissões do Ruphus vai além de leitura/escrita. Cada módulo tem permissões granulares para criar, editar, deletar, aprovar, exportar e configurar. Um vendedor pode ver oportunidades mas não aprovar pagamentos.</p><blockquote>Segurança por design significa que mesmo um bug na aplicação não expõe dados de outro tenant.</blockquote>"},
...
{t:"De 5 sistemas para 1 plataforma: o caso real de uma transportadora com 120 veículos",c:"Gestão",body:"<p>Como a consolidação de sistemas reduziu o fechamento mensal de 12 para 2 dias e liberou 40 horas/mês da equipe financeira.</p><h2>O problema</h2><p>A transportadora usava 5 sistemas diferentes: um para financeiro, outro para cobrança, planilhas para frete, WhatsApp pessoal para comunicação e banco para conciliação manual. Cada sistema com seu login, formato e limitações.</p><h2>A solução</h2><p>Migração para o Ruphus com 8 módulos: Financeiro, Contabilidade, Contas Bancárias, Aprovações, Chat, Relatórios, CRM e Integrações. Cobrança automatizada via WhatsApp Cloud API reduziu inadimplência de 14% para 5,8%.</p><blockquote>O fechamento que levava 12 dias agora leva 2. A equipe parou de digitar e começou a analisar.</blockquote>"},
...
{t:"7 erros ao escolher um ERP para sua empresa",c:"Gestão",body:"<p>De escolher pelo preço a ignorar integrações — os erros que custam meses de retrabalho.</p><h2>Os 7 erros</h2><p>1. Escolher só pelo preço (o barato sai caro). 2. Ignorar integrações (WhatsApp, bancos, NF-e). 3. Não testar com dados reais antes de contratar. 4. Subestimar o treinamento da equipe. 5. Contratar todos os módulos de uma vez. 6. Não verificar a frequência de atualizações. 7. Ignorar segurança e isolamento de dados.</p><p>O Ruphus resolve cada um: demonstração com dados reais, 13 integrações nativas, treinamento incluído, módulos sob demanda, atualizações semanais e Firestore Security Rules.</p>"},
...
{t:"Open Banking para empresas: como funciona e por que importa",c:"Economia",body:"<p>Extrato automático de 5 bancos, conciliação em 10 minutos e fim dos scrapers. O que muda com a API do Banco Central.</p><h2>Bancos suportados</h2><p>O Ruphus integra diretamente com Banco do Brasil, Bradesco, Caixa, Itaú e Santander via agregadores (Pluggy, Belvo, Plaid, TrueLayer). Sincronização automática de extratos, atualização de saldo em tempo real e gestão de consentimento.</p><h2>Diferença para scrapers</h2><p>Scrapers de terceiros violam termos de uso e podem quebrar a qualquer momento. A API via agregadores é regulada, segura e com webhooks para atualizações imediatas. Retry automático com backoff exponencial garante disponibilidade.</p>"},
{t:"PDV no ERP: por que separar o caixa do financeiro é um erro caro",c:"Produto",body:"<p>A maioria das empresas opera o PDV em um sistema isolado — e depois gasta horas conciliando vendas com o financeiro.</p><h2>Como o PDV do Ruphus funciona</h2><p>Frente de caixa completa com busca rápida (F2), múltiplas formas de pagamento, abertura e fechamento de sessão com conferência automática. Cada venda gera automaticamente o lançamento financeiro, a baixa no estoque e o registro fiscal.</p><h2>Modo Kiosk e atalhos</h2><p>O modo Kiosk (F11) transforma o navegador em terminal dedicado. Atalhos de teclado aceleram o atendimento. Descontos acima do limite exigem PIN de supervisor.</p><blockquote>Venda, estoque, financeiro e fiscal em um único clique.</blockquote>"},
{t:"Ponto eletrônico integrado: controle de horas sem sistemas externos",c:"Produto",body:"<p>Sistemas de ponto separados geram retrabalho no cálculo de horas extras e folha. Integrar o ponto ao ERP resolve na raiz.</p><h2>Registro e revisão</h2><p>Funcionário registra entrada, saída e intervalos. Gestor aprova batidas na revisão de folha mensal. Alertas automáticos para atrasos e faltas.</p><h2>Banco de horas automático</h2><p>Cálculo automático de extras e débitos. Métricas em tempo real para RH.</p><blockquote>O sistema calcula, alerta e aprova — o gestor só confere.</blockquote>"},
{t:"NFS-e e DANFE: emissão fiscal completa sem trocar de sistema",c:"Produto",body:"<p>Muitas empresas emitem NF-e de produtos mas precisam de outro sistema para NFS-e de serviços. O Ruphus resolve os dois via Focus NFe.</p><h2>NF-e + NFS-e + DANFE</h2><p>NF-e com autorização imediata na SEFAZ. NFS-e (RPS) para serviços com ISS. DANFE para visualização e download. Cancelamento em até 24h.</p><blockquote>Produto ou serviço, NF-e ou NFS-e — emita direto do ERP.</blockquote>"},
{t:"Gestão de serviços multi-nicho: do pet shop à clínica",c:"Produto",body:"<p>Cada negócio de serviços funciona diferente. O Ruphus resolve com sugestões pré-configuradas por nicho.</p><h2>11 nichos</h2><p>Estética, barbearia, pet shop, odontologia, fisioterapia, academia, oficina, imobiliária, saúde, educação e geral. Cada nicho sugere serviços, durações e preços típicos.</p><h2>Pools de capacidade e portal público</h2><p>O sistema checa profissional E recurso disponível. Clientes agendam pelo portal público sem criar conta.</p><blockquote>11 nichos, pools de capacidade, portal público e fila de espera.</blockquote>"},
{t:"Comunicação omnichannel: WhatsApp, Telegram, e-mail e SMS em um CRM",c:"Produto",body:"<p>4 canais com 4 ferramentas = perda de contexto. Centralizar no CRM muda o jogo.</p><h2>4 canais integrados</h2><p>WhatsApp Cloud API com chatbot e régua de cobrança. Telegram com comandos. Sequências de e-mail. SMS com 2FA. Histórico unificado por lead e cliente.</p><blockquote>4 canais, 1 histórico — tudo automático.</blockquote>"},
{t:"Copilot IA: o assistente que vive dentro do seu ERP",c:"Tecnologia",body:"<p>O Copilot do Ruphus vive em todas as telas e age proativamente.</p><h2>O que faz</h2><p>Classificação de documentos, composição de e-mails, sugestões de WhatsApp, priorização de notificações, analytics de RH. Model Router seleciona Claude, GPT ou Gemini automaticamente.</p><blockquote>Pergunte 'Quais fornecedores tiveram aumento de mais de 20%?' e receba a resposta em segundos.</blockquote>"},
{t:"Portal do Cliente: relatórios sem expor o ERP",c:"Produto",body:"<p>Clientes pedem DRE e balanço por e-mail. O Portal automatiza com link exclusivo.</p><h2>Acesso controlado</h2><p>Cada cliente visualiza apenas seus relatórios: DRE, Balanço, Balancete, Contas a Pagar/Receber e Lançamentos. Zero acesso ao ERP.</p><blockquote>O cliente se serve. Você para de ser garçom de relatórios.</blockquote>"},
{t:"Backup inteligente: proteção de dados enquanto você dorme",c:"Segurança",body:"<p>A diferença entre catástrofe e inconveniência é ter backup verificado.</p><h2>Como funciona</h2><p>Backups agendados (horário, diário, semanal, mensal). Seleção de módulos. Verificação de integridade. Controle de retenção com limpeza automática.</p><blockquote>Backup diário às 2h, verificação às 3h, alerta se falhar às 3h01.</blockquote>"},
{t:"Monitor360: o ERP que cuida do seu negócio — e da sua reputação",c:"Produto",body:"<p>O Monitor360 é o 28º módulo do Ruphus e inaugura uma nova categoria: Inteligência de Reputação. Pela primeira vez, um ERP brasileiro monitora redes sociais, notícias e avaliações em tempo real — integrado ao mesmo cadastro de clientes e às mesmas permissões.</p><h2>10+ fontes em um painel único</h2><p>Twitter/X, Instagram, YouTube, TikTok, Google Maps, Google News, NewsAPI, podcasts e fóruns. Coleta automática a cada 30 minutos com deduplicação por hash de conteúdo.</p><h2>IA analisa cada menção</h2><p>Claude classifica automaticamente o sentimento (positivo/negativo/neutro), define o nível de urgência e identifica menções que requerem resposta imediata. O Brand Health Score de 0 a 100 é recalculado diariamente.</p><h2>War Room para crises</h2><p>Quando o volume de menções negativas dispara, a sala virtual do War Room coordena a resposta: tarefas, responsáveis, SLA com countdown, timeline em tempo real e rascunhos de resposta assistidos por IA.</p><blockquote>Imagine receber um alerta no WhatsApp quando seu maior cliente posta uma reclamação no Google Maps — e já poder abrir um ticket e acompanhar a resolução sem sair do ERP.</blockquote>"},
{t:"Brand Health Score: como medir a saúde da marca em tempo real",c:"Tecnologia",body:"<p>O Brand Health Score do Monitor360 é um indicador de 0 a 100 que sintetiza a percepção do mercado sobre sua marca. Calculado diariamente com base em volume de menções, distribuição de sentimentos e velocidade de propagação.</p><h2>Como é calculado</h2><p>O algoritmo combina três dimensões: volume absoluto de menções (peso 30%), razão sentimento positivo/negativo (peso 50%) e velocidade de mudança nas últimas 24h (peso 20%). Normalizado entre 0 e 100 com benchmarks históricos.</p><h2>Predição de crises</h2><p>A IA detecta padrões de escalada antes da explosão: spike de volume, sentimento negativo crescente, velocidade anormal de propagação. O score de probabilidade vai de 0 a 1 com intervalo de confiança e recomendações de ação preventiva.</p><blockquote>Empresas que monitoram Brand Health Score respondem a crises 5x mais rápido — e em muitos casos, antes que virem crise de fato.</blockquote>"}
];
